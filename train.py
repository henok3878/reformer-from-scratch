...
def create_masks(
    src_batch: torch.Tensor, tgt_batch: torch.Tensor, padding_idx: int
    ...
    tgt_lower_matrix = torch.tril(
        torch.ones(size=(tgt_len, tgt_len), device=tgt_batch.device)
    ).bool()
...
class Trainer:
    def __init__(self, config: AppConfig, run_path: str):
        ...
        model = TransformerRoPE(
            model_config=self.config.model, data_config=self.config.data
        ).to(self.device)
        if IS_DDP:
        ...
        self.optimizer = torch.optim.AdamW(
            ...
            lr=self.config.training.lr_factor,
        ...
        self.full_val_loader = self._prepare_dataloader(
            dataset=self.full_val_dataset, shuffle=False
        )
        self.quick_val_loader = self._prepare_dataloader(
            self.quick_val_dataset, shuffle=False
        )
    ...
    def _load_tokenizers_and_datasets(self):
        ...
        if strategy == TokenizationStrategy.JOINT:
            ...
            processed_dir = (
                f"data/processed-joint-{self.config.data.subset}-vocab{vocab_size}"
            )
        ...
        self.train_dataset: Dataset = load_from_disk(train_dir, keep_in_memory=False)  # type: ignore
        self.full_val_dataset: Dataset = load_from_disk(val_dir, keep_in_memory=False)  # type: ignore
    ...
    def _prepare_dataloader(self, dataset, shuffle: bool):
        ...
        if IS_DDP:
            ...
            )  # only drop the final short batch in training
        return DataLoader(
            dataset,
    ...
    def _save_checkpoint(self, value: int, is_epoch: bool):
        ...
        model_state_dict = (
            self.model.module.state_dict() if IS_DDP else self.model.state_dict()
        )
    ...
    def _compute_bleu(self, loader: DataLoader) -> float:
        self.model.eval()
        local_hyps, local_refs = [], []

        model_to_eval = self.model.module if IS_DDP else self.model
        ...
        for batch in loader:
            ...
            # detokenise
            local_hyps.extend(
                self.tokenizer_tgt.decode_batch(preds, skip_special_tokens=True)
            )

            # references already tokenised, so detokenise once
            local_refs.extend(
                self.tokenizer_tgt.decode_batch(
        ...
        if not IS_DDP:
            bleu = sacrebleu.corpus_bleu(
                local_hyps, [local_refs], tokenize="13a", lowercase=False
            )
            self.model.train()
            return bleu.score
    ...
    def _eval_loader(self, loader: DataLoader):
        ...
        for batch in loader:
            ...
            loss = self.loss_fn(
                logits.reshape(-1, logits.size(-1)), batch["label"].reshape(-1)
            )
    ...
    def _run_full_validation(self):
        ...
        train_bleu = self._compute_bleu(self.quick_val_loader)
        if self.global_rank == 0:
            wandb.log(
                {
                    "val/full_loss": avg_loss,
                    "val/full_ppl": ppl,
                    "val/bleu": train_bleu,
            ...
            print(
                f"[FULL VAL] epoch={self.current_epoch} step={self.global_step} loss={avg_loss:.4f} ppl={ppl:.2f} bleu={train_bleu:.2f}"
            )
    ...
    def _run_quick_validation(self):
        ...
        if self.global_rank == 0:
            wandb.log(
                {
                    "val/quick_loss": avg_loss,
                    "val/quick_ppl": ppl,
                    "step": self.global_step,
                }
            )
            print(
                f"[QUICK VAL] epoch={self.current_epoch} step={self.global_step} loss={avg_loss:.4f} ppl={ppl:.2f}"
            )
        return avg_loss, ppl
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./configs/config_de-en.yaml",
    ...
    run_path: str
    is_main_process = not IS_DDP or int(os.environ["RANK"]) == 0
    if is_main_process:
